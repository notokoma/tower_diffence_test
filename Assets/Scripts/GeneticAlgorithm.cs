...
public class GeneticAlgorithm : MonoBehaviour
{
    ...
    float SurviveTimeMin,DestinationMin;
    int SurviveTimeMinNumber,DestinationMinNumber;
    ...
    public void GeneticStatus()
    {
        ...
        if(GameScore.StageNumber == 0){
            ...
            List<float> listSurviveTime = new List<float>(GameScore.EnemySurviveTime);
            List<float> listSurviveTimeCompare = listSurviveTime;
            listSurviveTime.Sort();
            //Debug.Log(String.Join(",", listSurviveTime.Select(n => n.ToString()))); 

            List<float> listDestination = new List<float>(GameScore.EnemyDestination);
            List<float> listDestinationCompare = listDestination;
            listDestination.Sort((a, b) => a.CompareTo(b));
            //Debug.Log(String.Join(",", listDestination.Select(n => n.ToString())));

            i = 0;
            while(i < 5){
                if(listSurviveTime[0] == listSurviveTimeCompare[i]){
                    SurviveTimeMin = listSurviveTime[0];
                    SurviveTimeMinNumber = i;
                }
                if(listDestination[0] == listDestinationCompare[i]){
                    DestinationMin = listDestination[0];
                    DestinationMinNumber = i;
                }
                i++;
            }
            print(listSurviveTime[0] + "," + SurviveTimeMinNumber);

            print(EnemyGeneticHP[SurviveTimeMinNumber] + "," + 
            EnemyPhysicsBuff[SurviveTimeMinNumber] + "," +
            EnemyMagicBuff[SurviveTimeMinNumber] + "," +
            EnemySpeedBuff[SurviveTimeMinNumber]);

            print(EnemyGeneticHP[DestinationMinNumber] + "," + 
            EnemyPhysicsBuff[DestinationMinNumber] + "," +
            EnemyMagicBuff[DestinationMinNumber] + "," +
            EnemySpeedBuff[DestinationMinNumber]);
